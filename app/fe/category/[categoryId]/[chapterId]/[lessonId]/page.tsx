...
export async function generateMetadata({
  ...
  const { categoryId, chapterId, lessonId } = await params;

  const record = await getFeLesson(categoryId, chapterId, lessonId);
...
export default async function FeLessonPage({ params }: FeLessonPageProps) {
  ...
  const { previous, next } = getFeLessonNavigation(chapter, lessonId);

  const chapterHref =
    `/fe/category/${encodeURIComponent(category.id)}` +
      ...
      <div className="mx-auto max-w-5xl px-5 py-7 sm:px-8 sm:py-10">
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            {previous ? (
              <ClientNavigation
                href={`${chapterHref}/${encodeURIComponent(previous.id)}`}
            ...
            {next ? (
              <ClientNavigation
                href={`${chapterHref}/${encodeURIComponent(next.id)}`}
              ...
              >
                ...
                <p className="mt-3 font-bold text-foreground">{next.titleJa}</p>
